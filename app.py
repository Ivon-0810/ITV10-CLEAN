...
@app.route("/parametres/postes", methods=["GET", "POST"])
@login_requis
@super_admin_requis
def parametres_postes():
    """Liste des postes/métiers disponibles à la création d'un compte —
    gérée exclusivement par le Super-Admin, partagée par toutes les
    sociétés (purement informatif, sans lien avec les permissions)."""
    conn = get_connection()
    if request.method == "POST":
        ...
        if not nom:
            ...
            try:
                conn.execute("INSERT INTO postes (nom, cree_par) VALUES (?,?)", (nom, session["email"]))
                conn.commit()
                log_audit("CREATION", "POSTES", nom)
                flash(f"Poste « {nom} » ajouté — disponible immédiatement à la création de comptes.", "success")
            except Exception:
                flash("Ce poste existe déjà.", "danger")

    postes_liste = conn.execute("SELECT * FROM postes ORDER BY nom").fetchall()
    conn.close()
    return render_template("parametres_postes.html", postes=postes_liste)


@app.route("/parametres/postes/<int:poste_id>/supprimer", methods=["POST"])
@login_requis
@super_admin_requis
def supprimer_poste(poste_id):
    conn = get_connection()
    poste = conn.execute("SELECT * FROM postes WHERE id = ?", (poste_id,)).fetchone()
    if poste:
        conn.execute("UPDATE users SET poste_id = NULL WHERE poste_id = ?", (poste_id,))
        conn.execute("DELETE FROM postes WHERE id = ?", (poste_id,))
        conn.commit()
        log_audit("SUPPRESSION", "POSTES", poste["nom"])
        flash(f"Poste « {poste['nom']} » supprimé.", "success")
    conn.close()
    return redirect(url_for("parametres_postes"))
...
# ------------------------------------------------------------------
# Catalogue des essences — extensible par les utilisateurs de terrain.
# Le catalogue global (tenant_id NULL) est visible par toutes les sociétés ;
# une société peut aussi ajouter ses propres essences, visibles uniquement
# par elle (cohérent avec l'autonomie stricte entre sociétés).
# ------------------------------------------------------------------

@app.route("/essences", methods=["GET", "POST"])
@login_requis
@roles_requis("PROSPECTEUR_FORESTIER", "CHEF_PARC_FORESTIER", "CHEF_PARC_USINE",
              "RESPONSABLE_SCIERIE", "ADMIN", "SUPER_ADMIN")
def essences():
    conn = get_connection()
    if request.method == "POST":
        nom = request.form.get("nom", "").strip()
        masse = request.form.get("masse_volumique_kg_m3", "").strip()
        dme = request.form.get("dme_cm", "").strip()
        if not nom or not masse or not dme:
            flash("Nom, masse volumique et DME sont obligatoires.", "danger")
        else:
            try:
                conn.execute(
                    "INSERT INTO essences (nom, masse_volumique_kg_m3, dme_cm, tenant_id) VALUES (?,?,?,?)",
                    (nom, float(masse), float(dme), session.get("tenant_id")),
                )
                conn.commit()
                log_audit("CREATION", "ESSENCES", nom)
                flash(f"Essence « {nom} » ajoutée au catalogue.", "success")
            except Exception:
                flash("Cette essence existe peut-être déjà (nom en doublon).", "danger")

    essences_globales = conn.execute(
        "SELECT * FROM essences WHERE tenant_id IS NULL ORDER BY nom"
    ).fetchall()
    essences_societe = []
    if session.get("tenant_id"):
        essences_societe = conn.execute(
            "SELECT * FROM essences WHERE tenant_id = ? ORDER BY nom", (session["tenant_id"],)
        ).fetchall()
    conn.close()
    return render_template("essences.html", essences_globales=essences_globales,
                            essences_societe=essences_societe)
...
def inventaire():
    ...
    essences = conn.execute("SELECT * FROM essences WHERE tenant_id IS NULL OR tenant_id = ? ORDER BY nom", (session["tenant_id"],)).fetchall()
    arbres = conn.execute(
...
# ------------------------------------------------------------------
# Modules scierie détaillés (V10) : Tronçonnage, Billonnage, Sciage de
# tête, Stock coursons, Production débitée journalière. Distincts du
# module « Scierie & rendements » (vue agrégée) et de « Transformation »
# (étapes génériques facultatives) — ici, la chaîne réelle observée sur
# le terrain, avec sa propre numérotation et ses propres formulaires.
# ------------------------------------------------------------------

def _essence_id_par_nom(conn, nom, tenant_id):
    if not nom:
        return None
    row = conn.execute(
        "SELECT id FROM essences WHERE lower(nom) = lower(?) AND (tenant_id IS NULL OR tenant_id = ?)",
        (nom, tenant_id),
    ).fetchone()
    return row["id"] if row else None


def _df10_id_par_numero(conn, numero, tenant_id):
    if not numero:
        return None
    row = conn.execute(
        "SELECT id FROM df10_registre WHERE numero_df10 = ? AND tenant_id = ?",
        (numero, tenant_id),
    ).fetchone()
    return row["id"] if row else None


@app.route("/tronconnage-scierie", methods=["GET", "POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "CHEF_PARC_USINE", "ADMIN", "SUPER_ADMIN", "AUDITEUR_CONTROLEUR")
def tronconnage_scierie():
    conn = get_connection()
    if request.method == "POST" and session["role"] != "AUDITEUR_CONTROLEUR":
        essence_id = int(request.form["essence_id"]) if request.form.get("essence_id") else None
        df10_id = _df10_id_par_numero(conn, request.form.get("numero_df10", "").strip(), session["tenant_id"])
        gb = request.form.get("gb_lv_cm") or None
        pb = request.form.get("pb_lv_cm") or None
        longueur = request.form.get("longueur_lv_m") or None
        volume = None
        if gb and pb and longueur:
            volume = volume_grume(float(gb), float(pb), float(longueur))
        conn.execute(
            """INSERT INTO troncons_scierie
               (tenant_id, date_arrivee, numero_lvg, provenance, essence_id, certification, classe,
                plaquette, df10_id, longueur_lv_m, gb_lv_cm, pb_lv_cm, dm_lv_cm, volume_lv_m3,
                parc_stockage, date_tronconnage, quart, destination, etat_bille, saisi_par)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (session["tenant_id"], request.form.get("date_arrivee"), request.form.get("numero_lvg"),
             request.form.get("provenance"), essence_id, request.form.get("certification"),
             request.form.get("classe"), request.form.get("plaquette"), df10_id,
             float(longueur) if longueur else None, float(gb) if gb else None, float(pb) if pb else None,
             request.form.get("dm_lv_cm") or None, volume, request.form.get("parc_stockage"),
             request.form.get("date_tronconnage"), request.form.get("quart"),
             request.form.get("destination"), request.form.get("etat_bille", "TRONCONNE"), session["email"]),
        )
        conn.commit()
        log_audit("CREATION", "TRONCONNAGE_SCIERIE", request.form.get("numero_lvg", ""))
        flash("Entrée de tronçonnage enregistrée.", "success")

    essences = conn.execute(
        "SELECT * FROM essences WHERE tenant_id IS NULL OR tenant_id = ? ORDER BY nom", (session["tenant_id"],)
    ).fetchall()
    lignes = conn.execute(
        "SELECT t.*, e.nom AS essence_nom FROM troncons_scierie t LEFT JOIN essences e ON e.id = t.essence_id "
        "WHERE t.tenant_id = ? ORDER BY t.id DESC LIMIT 100", (session["tenant_id"],)
    ).fetchall()
    conn.close()
    return render_template("tronconnage_scierie.html", essences=essences, lignes=lignes)


@app.route("/billonnage", methods=["GET", "POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "CHEF_PARC_USINE", "ADMIN", "SUPER_ADMIN", "AUDITEUR_CONTROLEUR")
def billonnage():
    conn = get_connection()
    if request.method == "POST" and session["role"] != "AUDITEUR_CONTROLEUR":
        essence_id = int(request.form["essence_id"]) if request.form.get("essence_id") else None
        df10_id = _df10_id_par_numero(conn, request.form.get("numero_df10", "").strip(), session["tenant_id"])
        dm = request.form.get("diametre_moyen_cm") or None
        longueur_cm = request.form.get("longueur_cm") or None
        volume = None
        if dm and longueur_cm:
            volume = volume_grume(float(dm), float(dm), float(longueur_cm) / 100.0)
        conn.execute(
            """INSERT INTO billons
               (tenant_id, date_billonnage, certif, lot_seq_cb, ordre_seq_cb, ordre_coursons, plaquette,
                longueur_cm, ordre_billon, essence_id, df10_id, code_courson, diametre_moyen_cm, volume_m3,
                code_barre_billon, saisi_par)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (session["tenant_id"], request.form.get("date_billonnage"), request.form.get("certif"),
             request.form.get("lot_seq_cb"), request.form.get("ordre_seq_cb"), request.form.get("ordre_coursons"),
             request.form.get("plaquette"), float(longueur_cm) if longueur_cm else None,
             request.form.get("ordre_billon"), essence_id, df10_id, request.form.get("code_courson"),
             float(dm) if dm else None, volume, request.form.get("code_barre_billon"), session["email"]),
        )
        conn.commit()
        log_audit("CREATION", "BILLONNAGE", request.form.get("plaquette", ""))
        flash("Billon enregistré.", "success")

    essences = conn.execute(
        "SELECT * FROM essences WHERE tenant_id IS NULL OR tenant_id = ? ORDER BY nom", (session["tenant_id"],)
    ).fetchall()
    lignes = conn.execute(
        "SELECT b.*, e.nom AS essence_nom FROM billons b LEFT JOIN essences e ON e.id = b.essence_id "
        "WHERE b.tenant_id = ? ORDER BY b.id DESC LIMIT 100", (session["tenant_id"],)
    ).fetchall()
    conn.close()
    return render_template("billonnage.html", essences=essences, lignes=lignes)


@app.route("/sciage-tete", methods=["GET", "POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "ADMIN", "SUPER_ADMIN", "AUDITEUR_CONTROLEUR")
def sciage_tete():
    conn = get_connection()
    if request.method == "POST" and session["role"] != "AUDITEUR_CONTROLEUR":
        ...
        df10_id = _df10_id_par_numero(conn, request.form.get("numero_df10", "").strip(), session["tenant_id"])
        conn.execute(
            """INSERT INTO sciage_tete
               (tenant_id, date_sciage, equipe, plaquette, df10_id, ordre_billon, longueur_finale_m,
                longueur_reelle_m, diametre_moyen_cm, volume_coursons_m3, essence_id, quart, saisi_par)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (session["tenant_id"], request.form.get("date_sciage"), request.form.get("equipe"),
             request.form.get("plaquette"), df10_id, request.form.get("ordre_billon"),
             request.form.get("longueur_finale_m") or None, request.form.get("longueur_reelle_m") or None,
             request.form.get("diametre_moyen_cm") or None, request.form.get("volume_coursons_m3") or None,
             essence_id, request.form.get("quart"), session["email"]),
        )
        conn.commit()
        log_audit("CREATION", "SCIAGE_TETE", request.form.get("plaquette", ""))
        flash("Sciage de tête enregistré.", "success")

    essences = conn.execute(
        "SELECT * FROM essences WHERE tenant_id IS NULL OR tenant_id = ? ORDER BY nom", (session["tenant_id"],)
    ).fetchall()
    lignes = conn.execute(
        "SELECT s.*, e.nom AS essence_nom FROM sciage_tete s LEFT JOIN essences e ON e.id = s.essence_id "
        "WHERE s.tenant_id = ? ORDER BY s.id DESC LIMIT 100", (session["tenant_id"],)
    ).fetchall()
    conn.close()
    return render_template("sciage_tete.html", essences=essences, lignes=lignes)


@app.route("/stock-coursons", methods=["GET", "POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "CHEF_PARC_USINE", "ADMIN", "SUPER_ADMIN", "AUDITEUR_CONTROLEUR")
def stock_coursons():
    conn = get_connection()
    if request.method == "POST" and session["role"] != "AUDITEUR_CONTROLEUR":
        essence_id = int(request.form["essence_id"]) if request.form.get("essence_id") else None
        df10_id = _df10_id_par_numero(conn, request.form.get("numero_df10", "").strip(), session["tenant_id"])
        gb, pb = request.form.get("gb_cm") or None, request.form.get("pb_cm") or None
        longueur_cm = request.form.get("longueur_cm") or None
        moyenne, volume = None, None
        if gb and pb:
            moyenne = round((float(gb) + float(pb)) / 2, 2)
        if moyenne and longueur_cm:
            volume = volume_grume(moyenne, moyenne, float(longueur_cm) / 100.0)
        conn.execute(
            """INSERT INTO stock_coursons
               (tenant_id, date_stock, essence_id, plaquette, df10_id, code_barre, numero_sequentiel,
                longueur_cm, gb_cm, pb_cm, moyenne_cm, volume_m3, saisi_par)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (session["tenant_id"], request.form.get("date_stock"), essence_id, request.form.get("plaquette"),
             df10_id, request.form.get("code_barre"), request.form.get("numero_sequentiel"),
             float(longueur_cm) if longueur_cm else None, float(gb) if gb else None, float(pb) if pb else None,
             moyenne, volume, session["email"]),
        )
        conn.commit()
        log_audit("CREATION", "STOCK_COURSONS", request.form.get("plaquette", ""))
        flash("Courson ajouté au stock.", "success")

    essences = conn.execute(
        "SELECT * FROM essences WHERE tenant_id IS NULL OR tenant_id = ? ORDER BY nom", (session["tenant_id"],)
    ).fetchall()
    lignes = conn.execute(
        "SELECT s.*, e.nom AS essence_nom FROM stock_coursons s LEFT JOIN essences e ON e.id = s.essence_id "
        "WHERE s.tenant_id = ? ORDER BY s.id DESC LIMIT 100", (session["tenant_id"],)
    ).fetchall()
    conn.close()
    return render_template("stock_coursons.html", essences=essences, lignes=lignes)


@app.route("/production-debitee", methods=["GET", "POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "ADMIN", "SUPER_ADMIN", "AUDITEUR_CONTROLEUR")
def production_debitee():
    conn = get_connection()
    if request.method == "POST" and session["role"] != "AUDITEUR_CONTROLEUR":
        essence_id = int(request.form["essence_id"]) if request.form.get("essence_id") else None
        pieces = request.form.get("pieces") or None
        longueur = request.form.get("longueur_mm") or None
        largeur = request.form.get("largeur_mm") or None
        epaisseur = request.form.get("epaisseur_mm") or None
        volume = None
        if pieces and longueur and largeur and epaisseur:
            volume = round(int(pieces) * (float(longueur) / 1000) * (float(largeur) / 1000) * (float(epaisseur) / 1000), 4)
        conn.execute(
            """INSERT INTO production_debitee
               (tenant_id, date_prod, quart, equipe, contrat_numero, contrat, ligne_prod, certification,
                numero_code_colis, numero_colis_emballage, essence_id, type_sechage, qualite, pieces,
                longueur_mm, largeur_mm, epaisseur_mm, volume_m3, saisi_par)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (session["tenant_id"], request.form.get("date_prod"), request.form.get("quart"),
             request.form.get("equipe"), request.form.get("contrat_numero"), request.form.get("contrat"),
             request.form.get("ligne_prod"), request.form.get("certification"),
             request.form.get("numero_code_colis"), request.form.get("numero_colis_emballage"), essence_id,
             request.form.get("type_sechage"), request.form.get("qualite"),
             int(pieces) if pieces else None, float(longueur) if longueur else None,
             float(largeur) if largeur else None, float(epaisseur) if epaisseur else None,
             volume, session["email"]),
        )
        conn.commit()
        log_audit("CREATION", "PRODUCTION_DEBITEE", request.form.get("numero_code_colis", ""))
    ...
    lignes = conn.execute(
        "SELECT p.*, e.nom AS essence_nom FROM production_debitee p LEFT JOIN essences e ON e.id = p.essence_id "
        "WHERE p.tenant_id = ? ORDER BY p.id DESC LIMIT 100", (session["tenant_id"],)
    ).fetchall()
    conn.close()
    return render_template("production_debitee.html", essences=essences, lignes=lignes)
...
RAPPORTS_EXCEL = {
    ...
    "tronconnage": {
        "titre": "Tronçonnage scierie",
        "entetes": ["N° LVG", "Provenance", "Essence", "Certification", "Classe", "Plaquette",
                    "Longueur LV (m)", "GB (cm)", "PB (cm)", "Volume LV (m³)", "Parc", "Date tronçonnage",
                    "Quart", "État bille", "Saisi par"],
        "requete": """SELECT t.numero_lvg, t.provenance, e.nom, t.certification, t.classe, t.plaquette,
                             t.longueur_lv_m, t.gb_lv_cm, t.pb_lv_cm, t.volume_lv_m3, t.parc_stockage,
                             t.date_tronconnage, t.quart, t.etat_bille, t.saisi_par
                      FROM troncons_scierie t LEFT JOIN essences e ON e.id = t.essence_id
                      WHERE t.tenant_id = ? ORDER BY t.id DESC""",
    },
    "billonnage": {
        "titre": "Billonnage",
        "entetes": ["Date", "Certif.", "Plaquette", "Ordre billon", "Essence", "Longueur (cm)",
                    "Diamètre moyen (cm)", "Volume (m³)", "Code barre", "Saisi par"],
        "requete": """SELECT b.date_billonnage, b.certif, b.plaquette, b.ordre_billon, e.nom,
                             b.longueur_cm, b.diametre_moyen_cm, b.volume_m3, b.code_barre_billon, b.saisi_par
                      FROM billons b LEFT JOIN essences e ON e.id = b.essence_id
                      WHERE b.tenant_id = ? ORDER BY b.id DESC""",
    },
    "sciage_tete": {
        "titre": "Sciage de tête",
        "entetes": ["Date", "Équipe", "Plaquette", "Ordre billon", "Longueur finale (m)",
                    "Longueur réelle (m)", "Diamètre moyen (cm)", "Volume coursons (m³)", "Essence",
                    "Quart", "Saisi par"],
        "requete": """SELECT s.date_sciage, s.equipe, s.plaquette, s.ordre_billon, s.longueur_finale_m,
                             s.longueur_reelle_m, s.diametre_moyen_cm, s.volume_coursons_m3, e.nom,
                             s.quart, s.saisi_par
                      FROM sciage_tete s LEFT JOIN essences e ON e.id = s.essence_id
                      WHERE s.tenant_id = ? ORDER BY s.id DESC""",
    },
    "stock_coursons": {
        "titre": "Stock coursons",
        "entetes": ["Date", "Essence", "Plaquette", "Code barre", "N° séquentiel", "Longueur (cm)",
                    "GB (cm)", "PB (cm)", "Moyenne (cm)", "Volume (m³)", "Saisi par"],
        "requete": """SELECT sc.date_stock, e.nom, sc.plaquette, sc.code_barre, sc.numero_sequentiel,
                             sc.longueur_cm, sc.gb_cm, sc.pb_cm, sc.moyenne_cm, sc.volume_m3, sc.saisi_par
                      FROM stock_coursons sc LEFT JOIN essences e ON e.id = sc.essence_id
                      WHERE sc.tenant_id = ? ORDER BY sc.id DESC""",
    },
    "production_debitee": {
        "titre": "Production débitée journalière",
        "entetes": ["Date", "Quart", "Équipe", "Contrat", "Ligne", "Certification", "N° colis", "Essence",
                    "Type séchage", "Qualité", "Pièces", "Longueur (mm)", "Largeur (mm)", "Épaisseur (mm)",
                    "Volume (m³)", "Saisi par"],
        "requete": """SELECT p.date_prod, p.quart, p.equipe, p.contrat, p.ligne_prod, p.certification,
                             p.numero_code_colis, e.nom, p.type_sechage, p.qualite, p.pieces, p.longueur_mm,
                             p.largeur_mm, p.epaisseur_mm, p.volume_m3, p.saisi_par
                      FROM production_debitee p LEFT JOIN essences e ON e.id = p.essence_id
...
IMPORT_MODELES = {
    ...
    "tronconnage": ["date_arrivee", "numero_lvg", "provenance", "essence_nom", "certification", "classe",
                     "plaquette", "numero_df10", "longueur_lv_m", "gb_lv_cm", "pb_lv_cm", "dm_lv_cm",
                     "parc_stockage", "date_tronconnage", "quart", "destination", "etat_bille"],
    "billonnage": ["date_billonnage", "certif", "lot_seq_cb", "ordre_seq_cb", "ordre_coursons", "plaquette",
                    "longueur_cm", "ordre_billon", "essence_nom", "numero_df10", "code_courson",
                    "diametre_moyen_cm", "code_barre_billon"],
    "sciage_tete": ["date_sciage", "equipe", "plaquette", "numero_df10", "ordre_billon", "longueur_finale_m",
                     "longueur_reelle_m", "diametre_moyen_cm", "volume_coursons_m3", "essence_nom", "quart"],
    "stock_coursons": ["date_stock", "essence_nom", "plaquette", "numero_df10", "code_barre",
                        "numero_sequentiel", "longueur_cm", "gb_cm", "pb_cm"],
    "production_debitee": ["date_prod", "quart", "equipe", "contrat_numero", "contrat", "ligne_prod",
                            "certification", "numero_code_colis", "numero_colis_emballage", "essence_nom",
                            "type_sechage", "qualite", "pieces", "longueur_mm", "largeur_mm", "epaisseur_mm"],
}
...
def import_excel_inventaire():
    ...
    essences_par_nom = {row["nom"].strip().lower(): row["id"] for row in
                         conn.execute("SELECT id, nom FROM essences WHERE tenant_id IS NULL OR tenant_id = ?", (session["tenant_id"],)).fetchall()}
...
@app.route("/tronconnage-scierie/import-excel", methods=["POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "CHEF_PARC_USINE", "ADMIN", "SUPER_ADMIN")
def import_excel_tronconnage():
    fichier = request.files.get("fichier")
    if not fichier:
        flash("Aucun fichier sélectionné.", "danger")
        return redirect(url_for("tronconnage_scierie"))
    colonnes = ["date_arrivee", "numero_lvg", "provenance", "essence_nom", "certification", "classe",
                "plaquette", "numero_df10", "longueur_lv_m", "gb_lv_cm", "pb_lv_cm", "dm_lv_cm",
                "parc_stockage", "date_tronconnage", "quart", "destination", "etat_bille"]
    try:
        lignes = importer_xlsx(fichier.read(), colonnes)
    ...
    for ligne in lignes:
        try:
            essence_id = _essence_id_par_nom(conn, ligne.get("essence_nom"), session["tenant_id"])
            df10_id = _df10_id_par_numero(conn, ligne.get("numero_df10"), session["tenant_id"])
            gb, pb, longueur = ligne.get("gb_lv_cm"), ligne.get("pb_lv_cm"), ligne.get("longueur_lv_m")
            volume = None
            if gb and pb and longueur:
                volume = volume_grume(float(gb), float(pb), float(longueur))
            conn.execute(
                """INSERT INTO troncons_scierie
                   (tenant_id, date_arrivee, numero_lvg, provenance, essence_id, certification, classe,
                    ...
                    parc_stockage, date_tronconnage, quart, destination, etat_bille, saisi_par)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (session["tenant_id"], ligne.get("date_arrivee"), ligne.get("numero_lvg"), ligne.get("provenance"),
                 essence_id, ligne.get("certification"), ligne.get("classe"), ligne.get("plaquette"), df10_id,
                 float(longueur) if longueur else None, float(gb) if gb else None, float(pb) if pb else None,
                 ligne.get("dm_lv_cm") or None, volume, ligne.get("parc_stockage"),
                 ligne.get("date_tronconnage"), ligne.get("quart"), ligne.get("destination"),
                 ligne.get("etat_bille") or "TRONCONNE", session["email"]),
            )
            nb_ok += 1
        except (ValueError, TypeError):
            nb_erreurs += 1
    conn.commit()
    conn.close()
    log_audit("IMPORT_EXCEL", "TRONCONNAGE_SCIERIE", f"{nb_ok} lignes importées, {nb_erreurs} erreurs")
    flash(f"{nb_ok} entrée(s) importée(s), {nb_erreurs} ligne(s) ignorée(s).", "success" if nb_erreurs == 0 else "warning")
    return redirect(url_for("tronconnage_scierie"))


@app.route("/billonnage/import-excel", methods=["POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "CHEF_PARC_USINE", "ADMIN", "SUPER_ADMIN")
def import_excel_billonnage():
    fichier = request.files.get("fichier")
    if not fichier:
        flash("Aucun fichier sélectionné.", "danger")
        return redirect(url_for("billonnage"))
    colonnes = ["date_billonnage", "certif", "lot_seq_cb", "ordre_seq_cb", "ordre_coursons", "plaquette",
                "longueur_cm", "ordre_billon", "essence_nom", "numero_df10", "code_courson",
                "diametre_moyen_cm", "code_barre_billon"]
    try:
        lignes = importer_xlsx(fichier.read(), colonnes)
    ...
    for ligne in lignes:
        try:
            essence_id = _essence_id_par_nom(conn, ligne.get("essence_nom"), session["tenant_id"])
            df10_id = _df10_id_par_numero(conn, ligne.get("numero_df10"), session["tenant_id"])
            dm, longueur_cm = ligne.get("diametre_moyen_cm"), ligne.get("longueur_cm")
            volume = None
            if dm and longueur_cm:
                volume = volume_grume(float(dm), float(dm), float(longueur_cm) / 100.0)
            conn.execute(
                """INSERT INTO billons
                   (tenant_id, date_billonnage, certif, lot_seq_cb, ordre_seq_cb, ordre_coursons, plaquette,
                    ...
                    code_barre_billon, saisi_par)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (session["tenant_id"], ligne.get("date_billonnage"), ligne.get("certif"), ligne.get("lot_seq_cb"),
                 ligne.get("ordre_seq_cb"), ligne.get("ordre_coursons"), ligne.get("plaquette"),
                 float(longueur_cm) if longueur_cm else None, ligne.get("ordre_billon"), essence_id, df10_id,
                 ligne.get("code_courson"), float(dm) if dm else None, volume,
                 ligne.get("code_barre_billon"), session["email"]),
            )
            nb_ok += 1
        except (ValueError, TypeError):
            nb_erreurs += 1
    conn.commit()
    conn.close()
    log_audit("IMPORT_EXCEL", "BILLONNAGE", f"{nb_ok} lignes importées, {nb_erreurs} erreurs")
    flash(f"{nb_ok} billon(s) importé(s), {nb_erreurs} ligne(s) ignorée(s).", "success" if nb_erreurs == 0 else "warning")
    return redirect(url_for("billonnage"))


@app.route("/sciage-tete/import-excel", methods=["POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "ADMIN", "SUPER_ADMIN")
def import_excel_sciage_tete():
    fichier = request.files.get("fichier")
    if not fichier:
        flash("Aucun fichier sélectionné.", "danger")
        return redirect(url_for("sciage_tete"))
    colonnes = ["date_sciage", "equipe", "plaquette", "numero_df10", "ordre_billon", "longueur_finale_m",
                "longueur_reelle_m", "diametre_moyen_cm", "volume_coursons_m3", "essence_nom", "quart"]
    try:
        lignes = importer_xlsx(fichier.read(), colonnes)
    except ValueError as exc:
            ...
            conn.execute(
                """INSERT INTO sciage_tete
                   (tenant_id, date_sciage, equipe, plaquette, df10_id, ordre_billon, longueur_finale_m,
                    longueur_reelle_m, diametre_moyen_cm, volume_coursons_m3, essence_id, quart, saisi_par)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (session["tenant_id"], ligne.get("date_sciage"), ligne.get("equipe"), ligne.get("plaquette"),
                 df10_id, ligne.get("ordre_billon"), ligne.get("longueur_finale_m") or None,
                 ligne.get("longueur_reelle_m") or None, ligne.get("diametre_moyen_cm") or None,
                 ligne.get("volume_coursons_m3") or None, essence_id, ligne.get("quart"), session["email"]),
            )
            nb_ok += 1
        except (ValueError, TypeError):
            nb_erreurs += 1
    conn.commit()
    conn.close()
    log_audit("IMPORT_EXCEL", "SCIAGE_TETE", f"{nb_ok} lignes importées, {nb_erreurs} erreurs")
    flash(f"{nb_ok} sciage(s) importé(s), {nb_erreurs} ligne(s) ignorée(s).", "success" if nb_erreurs == 0 else "warning")
    return redirect(url_for("sciage_tete"))


@app.route("/stock-coursons/import-excel", methods=["POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "CHEF_PARC_USINE", "ADMIN", "SUPER_ADMIN")
def import_excel_stock_coursons():
    fichier = request.files.get("fichier")
    if not fichier:
        flash("Aucun fichier sélectionné.", "danger")
        return redirect(url_for("stock_coursons"))
    colonnes = ["date_stock", "essence_nom", "plaquette", "numero_df10", "code_barre",
                "numero_sequentiel", "longueur_cm", "gb_cm", "pb_cm"]
    try:
        lignes = importer_xlsx(fichier.read(), colonnes)
    except ValueError as exc:
    ...
    for ligne in lignes:
        try:
            essence_id = _essence_id_par_nom(conn, ligne.get("essence_nom"), session["tenant_id"])
            df10_id = _df10_id_par_numero(conn, ligne.get("numero_df10"), session["tenant_id"])
            gb, pb, longueur_cm = ligne.get("gb_cm"), ligne.get("pb_cm"), ligne.get("longueur_cm")
            moyenne, volume = None, None
            if gb and pb:
                moyenne = round((float(gb) + float(pb)) / 2, 2)
            if moyenne and longueur_cm:
            ...
            conn.execute(
                """INSERT INTO stock_coursons
                   (tenant_id, date_stock, essence_id, plaquette, df10_id, code_barre, numero_sequentiel,
                    longueur_cm, gb_cm, pb_cm, moyenne_cm, volume_m3, saisi_par)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (session["tenant_id"], ligne.get("date_stock"), essence_id, ligne.get("plaquette"), df10_id,
                 ligne.get("code_barre"), ligne.get("numero_sequentiel"),
                 float(longueur_cm) if longueur_cm else None, float(gb) if gb else None,
                 float(pb) if pb else None, moyenne, volume, session["email"]),
            )
            nb_ok += 1
        except (ValueError, TypeError):
            nb_erreurs += 1
    conn.commit()
    conn.close()
    log_audit("IMPORT_EXCEL", "STOCK_COURSONS", f"{nb_ok} lignes importées, {nb_erreurs} erreurs")
    flash(f"{nb_ok} entrée(s) de stock importée(s), {nb_erreurs} ligne(s) ignorée(s).", "success" if nb_erreurs == 0 else "warning")
    return redirect(url_for("stock_coursons"))


@app.route("/production-debitee/import-excel", methods=["POST"])
@login_requis
@roles_requis("RESPONSABLE_SCIERIE", "ADMIN", "SUPER_ADMIN")
def import_excel_production_debitee():
    fichier = request.files.get("fichier")
    if not fichier:
        flash("Aucun fichier sélectionné.", "danger")
        return redirect(url_for("production_debitee"))
    colonnes = ["date_prod", "quart", "equipe", "contrat_numero", "contrat", "ligne_prod", "certification",
                "numero_code_colis", "numero_colis_emballage", "essence_nom", "type_sechage", "qualite",
                "pieces", "longueur_mm", "largeur_mm", "epaisseur_mm"]
    try:
        lignes = importer_xlsx(fichier.read(), colonnes)
    except ValueError as exc:
        flash(str(exc), "danger")
    ...
    for ligne in lignes:
        try:
            essence_id = _essence_id_par_nom(conn, ligne.get("essence_nom"), session["tenant_id"])
            pieces = ligne.get("pieces")
            longueur, largeur, epaisseur = ligne.get("longueur_mm"), ligne.get("largeur_mm"), ligne.get("epaisseur_mm")
            volume = None
            if pieces and longueur and largeur and epaisseur:
                volume = round(int(float(pieces)) * (float(longueur)/1000) * (float(largeur)/1000) * (float(epaisseur)/1000), 4)
            conn.execute(
                """INSERT INTO production_debitee
                   (tenant_id, date_prod, quart, equipe, contrat_numero, contrat, ligne_prod, certification,
                    numero_code_colis, numero_colis_emballage, essence_id, type_sechage, qualite, pieces,
                    longueur_mm, largeur_mm, epaisseur_mm, volume_m3, saisi_par)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (session["tenant_id"], ligne.get("date_prod"), ligne.get("quart"), ligne.get("equipe"),
                 ligne.get("contrat_numero"), ligne.get("contrat"), ligne.get("ligne_prod"),
                 ligne.get("certification"), ligne.get("numero_code_colis"), ligne.get("numero_colis_emballage"),
                 essence_id, ligne.get("type_sechage"), ligne.get("qualite"),
                 int(float(pieces)) if pieces else None, float(longueur) if longueur else None,
                 float(largeur) if largeur else None, float(epaisseur) if epaisseur else None,
                 volume, session["email"]),
            )
            nb_ok += 1
    ...
    conn.commit()
    conn.close()
    log_audit("IMPORT_EXCEL", "PRODUCTION_DEBITEE", f"{nb_ok} lignes importées, {nb_erreurs} erreurs")
    flash(f"{nb_ok} production(s) importée(s), {nb_erreurs} ligne(s) ignorée(s).", "success" if nb_erreurs == 0 else "warning")
    return redirect(url_for("production_debitee"))
...
def super_admin_utilisateurs():
    ...
    utilisateurs = conn.execute(
        "SELECT u.*, t.nom AS tenant_nom, p.nom AS poste_nom FROM users u "
        "LEFT JOIN tenants t ON t.id = u.tenant_id LEFT JOIN postes p ON p.id = u.poste_id "
        "ORDER BY t.nom, u.role, u.nom_complet"
    ).fetchall()
    postes_disponibles = conn.execute("SELECT * FROM postes ORDER BY nom").fetchall()
    conn.close()
...
def super_admin_creer_utilisateur():
    ...
    email = request.form.get("email", "").strip().lower()
    poste_id = request.form.get("poste_id") or None
    ...
    try:
        conn.execute(
            "INSERT INTO users (nom_complet, email, password_hash, role, tenant_id, actif, doit_changer_mdp, cree_par, poste_id) "
            "VALUES (?,?,?,?,?,1,1,?,?)",
            (nom, email, hash_password(mot_de_passe), role, tenant_id, session["email"], poste_id),
        )
        conn.commit()
...
def utilisateurs_societe():
    conn = get_connection()
    utilisateurs = conn.execute(
        "SELECT u.*, p.nom AS poste_nom FROM users u LEFT JOIN postes p ON p.id = u.poste_id "
        "WHERE u.tenant_id = ? ORDER BY u.role, u.nom_complet", (session["tenant_id"],)
    ).fetchall()
    postes_disponibles = conn.execute("SELECT * FROM postes ORDER BY nom").fetchall()
    conn.close()
...
def creer_utilisateur_societe():
    ...
    try:
        conn.execute(
            "INSERT INTO users (nom_complet, email, password_hash, role, tenant_id, actif, doit_changer_mdp, cree_par, poste_id) "
            "VALUES (?,?,?,?,?,1,1,?,?)",
            (nom, email, hash_password(mot_de_passe), role, session["tenant_id"], session["email"], poste_id),
        )
        conn.commit()
