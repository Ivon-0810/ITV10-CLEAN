...
def _migrer_colonnes_additionnelles(conn):
    ...
    # ---- V10 : postes personnalisés + modules scierie détaillés ----

    # postes : intitulés de poste/métier définis par le Super-Admin, sélectionnables
    # lors de la création d'un compte (purement informatif, n'accorde aucun droit —
    # le rôle système continue seul de gouverner les permissions).
    conn.execute(
    ...
    if not _colonne_existe(conn, "users", "poste_id"):
        conn.execute("ALTER TABLE users ADD COLUMN poste_id INTEGER REFERENCES postes(id)")

    # billons : cubage et numérotation des billons issus du tronçonnage.
    conn.execute(
        """CREATE TABLE IF NOT EXISTS billons (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            tenant_id INTEGER NOT NULL,
            date_billonnage TEXT,
            certif TEXT,
            lot_seq_cb TEXT,
            ordre_seq_cb TEXT,
            ordre_coursons TEXT,
            ...
            df10_id INTEGER,
            code_courson TEXT,
            diametre_moyen_cm REAL,
            volume_m3 REAL,
            code_barre_billon TEXT,
            saisi_par TEXT,
            created_at TEXT DEFAULT (datetime('now')),
    ...
    # troncons_scierie : tronçonnage détaillé au niveau scierie (arrivée LVG,
    # dimensions grume, destination), distinct du module Transformation générique.
    conn.execute(
        """CREATE TABLE IF NOT EXISTS troncons_scierie (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            tenant_id INTEGER NOT NULL,
            date_arrivee TEXT,
            numero_lvg TEXT,
    ...
    # sciage_tete : passage du billon au courson (scie de tête).
    conn.execute(
        """CREATE TABLE IF NOT EXISTS sciage_tete (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            tenant_id INTEGER NOT NULL,
            date_sciage TEXT,
            equipe TEXT,
            ...
            df10_id INTEGER,
            ordre_billon TEXT,
            longueur_finale_m REAL,
            longueur_reelle_m REAL,
            diametre_moyen_cm REAL,
            volume_coursons_m3 REAL,
            essence_id INTEGER,
            quart TEXT,
            saisi_par TEXT,
            created_at TEXT DEFAULT (datetime('now')),
    ...
    # stock_coursons : inventaire des coursons en stock (avant sciage secondaire).
    conn.execute(
        """CREATE TABLE IF NOT EXISTS stock_coursons (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            tenant_id INTEGER NOT NULL,
            date_stock TEXT,
            essence_id INTEGER,
    ...
    # production_debitee : production journalière fine (colis, dimensions, qualité).
    conn.execute(
        """CREATE TABLE IF NOT EXISTS production_debitee (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            tenant_id INTEGER NOT NULL,
            date_prod TEXT,
            quart TEXT,
            ...
            ligne_prod TEXT,
            certification TEXT,
            numero_code_colis TEXT,
            numero_colis_emballage TEXT,
            essence_id INTEGER,
            type_sechage TEXT,
            qualite TEXT,
            pieces INTEGER,
